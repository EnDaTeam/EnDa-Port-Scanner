...
while mainp:
    ...
    while True:
        ...
        tip("Leave blank the port limit if you want to scan a specific port!",option=1)
        enter()
    ...
    try:
        ...
        time.sleep(2)
        enter()   
        repetitive = input(Colorate.Horizontal(Colors.red_to_white,"[!] Do you want to do another scan? >> ",2))
        if repetitive.lower() in ("1","y","yeah","yes","","ye","yep"):
            continue
        else:
            enter()
            print(Fore.RED + "[-] Roger that, exiting program.." + Fore.RESET)
            time.sleep(3)
            exit()
